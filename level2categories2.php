<?php
/*
Plugin Name: Level2Categories 2
Plugin URI: http://www.pirex.com.br/wordpress-plugins/
Description: Allows you to create a relationship between User Levels and Categories, so only users with a defined level will be able to post on a chosen category. This version works only with latests versions of wordpress, in wich taxonomy is present. 
Author: LeoGermani
Version: 2.2.1
...
function levels2categories_admin_page() {
	...
	$query=mysql_query("SELECT $l2c_category_table.term_id as ID, $l2c_category_table_name.name FROM $l2c_category_table INNER JOIN $l2c_category_table_name ON $l2c_category_table.term_id = $l2c_category_table_name.term_id WHERE $l2c_category_table.taxonomy = 'category' ");
	...
	$querystring="SELECT ".$l2c_table.".cat_ID, ".$l2c_table.".level, ".$l2c_category_table_name.".name as cat_name FROM ".$l2c_table." INNER JOIN ".$l2c_category_table_name." ON ".$l2c_table.".cat_ID = ".$l2c_category_table_name.".term_id";		
